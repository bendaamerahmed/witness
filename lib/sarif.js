...
const RULES = {
  ...
  'deleted check': {
    id: 'witness/deleted-check',
    name: 'DeletedCheck',
    short: 'A test was removed and no source file changed',
    full: 'One or more tests were deleted in a change that touched no source file. Every other rule here inspects added lines, so a check removed outright leaves nothing to inspect — this is the one shape a diff-based detector misses by construction unless it is looked for directly. Deleting tests is frequently legitimate: obsolete cases, merged duplicates, a feature removed along with its coverage. That is why the rule requires that no source file changed, and why it is not in the scanner default.',
  },
  'fixture fitting': {
...
/**
 * The anchor comes from the TELL, not the display name.
 *
 * docs/TELLS.md is organised one section per tell — `## swallow`, `## skip` —
 * while the SARIF display names are longer (`SwallowedError`, `SkippedTest`).
 * Building the anchor from the display name left those two helpUris pointing at
 * headings that do not exist, since v0.2.0: GitHub renders the link in every
 * code scanning alert, and a missing anchor lands silently at the top of the
 * page. Keyed on the tell, all eight resolve.
 */
const anchorFor = (tell) => String(tell).toLowerCase().replace(/[^a-z0-9]/g, '');

function rulesArray() {
  return Object.entries(RULES).map(([tell, r]) => ({
    id: r.id,
    name: r.name,
    ...
    helpUri: `${INFO_URI}/blob/main/docs/TELLS.md#${anchorFor(tell)}`,
  }));
}
