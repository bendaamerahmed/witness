...
**Disagreeing with a wild verdict is a welcome pull request.** [`benchmarks/wild-labels.json`](benchmarks/wild-labels.json) is one rater's opinion and that rater maintains the tool. Flip a verdict, write why in the `why` field, leave the old reasoning in the commit message. A second opinion on those 34 findings is worth more to this project than a new tell.
