...
name: witness-scan
description: >
  Scan the working diff for the six ways a green check lies: softened assertions,
  suppression comments, skip markers, swallowed errors, no-op fixes that touch only
  tests, and branches fitted to the test fixture. Reports each with the file, line,
...
3. Walk each **added** line for the six tells. Removed lines and pre-existing code are not this diff's problem.
...
Order by how much the finding hides. `no-op fix` first, then `softened assertion`, then `swallow`, then `skip`, then `suppression`, then `fixture fitting`. A suppression is visible in the file forever. A softened assertion looks like a passing test.
